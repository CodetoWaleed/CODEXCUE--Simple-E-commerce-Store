...
const Slug = ({ buyNow, addToCart, product, variants, error }) => {
  ...
  const checkPinAvail = async () => {
    const pins = await fetch(`${process.env.NEXT_PUBLIC_HOST}/api/pincode`);
    const pinJson = await pins.json();

    // console.log(pinJson, pinValue)
    if (Object.keys(pinJson).includes(pinValue)) {
      setservice(true);
      toast.success("Congratulation! We are delivering to this Pincode", {
      ...
      setservice(false);
      toast.error("Sorry! Pincode not Deliverable", {
        position: "bottom-center",
        autoClose: 1000,
        hideProgressBar: false,
        closeOnClick: true,
      ...
      <section className="text-gray-600 body-font overflow-hidden">
        <div className="container px-5 py-16 mx-auto">
          <div className="lg:w-4/5 mx-auto flex flex-wrap">
            <img
              alt="ecommerce"
              className="lg:w-1/4 w-full lg:h-auto m-auto xs:px-24 md:h-[40vh] block object-cover object-top rounded"
              src={product.img}
            />
            <div className="lg:w-1/2 w-full lg:pl-10 lg:py-6 mt-6 lg:mt-0">
                    ...
                    Object.keys(variants["red"]).includes(size) && (
                      <button
                        ...
                        className={`border-2 ml-1 bg-red-700 rounded-full w-6 h-6 focus:outline-none ${
                          color === "red" ? "border-red-700" : "border-white"
                        }`}
                      ></button>
                    )}
                    ...
                    Object.keys(variants["blue"]).includes(size) && (
                      <button
                        ...
                        className={`border-2 ml-1 bg-blue-700 rounded-full w-6 h-6 focus:outline-none ${
                          color === "blue" ? "border-blue-700" : "border-white"
                        }`}
                      ></button>
                    )}
                ...
                <div className="flex ml-6 items-center">
                  ...
                  <div className="relative">
                    <select
                      ...
                      className="rounded border appearance-none py-2 focus:outline-none focus:ring-2 focus:ring-black focus:border-black text-base pl-3 pr-10"
                      id="select"
                      value={size}
                    >
              ...
              <div className="flex">
                ...
                <button
                  ...
                  className="flex md:ml-10 ml-2 md:tracking-wider text-white bg-black border-0 py-2 md:px-6 px-2 focus:outline-none hover:bg-black rounded disabled:bg-gray-700"
                >
                  Buy Now
                </button>
                <button
                  ...
                  className="flex ml-2 md:tracking-wider text-white bg-black border-0 py-2 md:px-6 px-2 focus:outline-none hover:bg-black rounded disabled:bg-gray-700"
                >
                  Add to Cart
                </button>
              ...
              <div className="pin flex mt-6">
                <input
                  type="text"
                  className="border-2 border-black w-1/2 px-2"
                  onChange={onChangePin}
                  name="pinValue"
                  placeholder="Enter your Pin Code"
                />
                <button
                  onClick={checkPinAvail}
                  className="flex ml-2 text-white bg-black border-0 py-2 px-6 focus:outline-none hover:bg-black rounded"
                >
                  Check
                </button>
              </div>
              ...
              {service && service != null && (
                <div className="text-green-700">
                  CONGRATULATIONS ! We are delivering to this Pincode
                </div>
              )}
            </div>
          </div>
...
export async function getServerSideProps(context) {
  ...
  let colorSizeSlug = {}; // {red: {xl: {slug: "wear-the-premium"}}}
